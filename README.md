...
This project has been restructured to be more organized and scalable.

-   `solutions/`: Contains the Python scripts for each solved problem. Each file is named `ID_n.py`, where `n` is the problem number.
-   `tests/`: Contains the unit tests for the solutions. The tests are written using Python's built-in `unittest` framework.
